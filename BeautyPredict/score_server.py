...
class IndexHandler(tornado.web.RequestHandler, ABC):
    ...
    def post(self):
        ...
        if not url:
            ...
            _conn.execute("INSERT INTO upload_images (uuid, file_name, image_data, score) VALUES (?,?,?,?)",
                          (image_uuid, url,data,score))
            _conn.commit()
        _conn.close()
        self.render("image.html", image_uuid=image_uuid, scores=scores)
