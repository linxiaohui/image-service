...
class IndexHandler(tornado.web.RequestHandler, ABC):
    ...
    def post(self):
        ...
        if not url:
            ...
            _conn.execute("INSERT INTO upload_images (uuid, file_name, image_data, image_sketch) VALUES (?,?,?,?)",
...
class ImageSketchHandler(tornado.web.RequestHandler, ABC):
    def get(self, gen_uuid):
        _conn = sqlite3.connect("image.db")
        _cursor = _conn.cursor()
        _cursor.execute("SELECT image_sketch FROM  upload_images WHERE uuid=?", (gen_uuid,))
        image = _cursor.fetchone()
        _conn.close()
        if image:
            self.write(image[0])
...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        handlers = [
            ...
            (r"/image_sketch/(.+)", ImageSketchHandler),
...
if __name__ == "__main__":
    try:
        ...
        conn.execute("""CREATE TABLE upload_images(image_id INTEGER PRIMARY KEY AUTOINCREMENT, uuid TEXT, 
                                                   file_name TEXT, image_data BLOB, image_sketch BLOB,
