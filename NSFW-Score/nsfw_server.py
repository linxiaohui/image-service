    ...
    def post(self):
        ...
        if not file_metas:
            ...
            for meta in file_metas:
                ...
                data = meta['body']
                _data = nsfw_score(data)
                image_uuid = str(uuid.uuid4())
                _conn.execute("INSERT INTO upload_images (uuid, file_name, image_data, nsfw_score) VALUES (?,?,?,?)",
                              (image_uuid, filename, data,_data))
                _conn.commit()
        _conn.close()
        self.render("image.html", image_uuid=image_uuid, nsfw_score=_data)
...
if __name__ == "__main__":
    try:
        ...
        conn.execute("""CREATE TABLE upload_images(image_id INTEGER PRIMARY KEY AUTOINCREMENT, uuid TEXT, 
                                                   file_name TEXT, image_data BLOB, nsfw_score float,
