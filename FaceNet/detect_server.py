...
class IndexHandler(tornado.web.RequestHandler, ABC):
    ...
    def post(self):
        ...
        if not url:
            ...
            data = resp.content
            _data = DETECTOR.mark_faces(data)
            image_uuid = str(uuid.uuid4())
            _conn.execute("INSERT INTO upload_images (uuid, file_name, image_data, image_face) VALUES (?,?,?,?)",
...
class ImageFaceHandler(tornado.web.RequestHandler, ABC):
    def get(self, gen_uuid):
        _conn = sqlite3.connect("image.db")
        _cursor = _conn.cursor()
        _cursor.execute("SELECT image_face FROM  upload_images WHERE uuid=?", (gen_uuid,))
        image = _cursor.fetchone()
        _conn.close()
        if image:
            self.write(image[0])
...
if __name__ == "__main__":
    try:
        ...
        conn.execute("""CREATE TABLE upload_images(image_id INTEGER PRIMARY KEY AUTOINCREMENT, uuid TEXT, 
                                                   file_name TEXT, image_data BLOB, image_face BLOB,
