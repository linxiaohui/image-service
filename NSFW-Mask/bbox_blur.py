...
def analyze_box(new_hp, f1=1.0, f2=0.0):
    ...
    # (x_index_ndarray,y_index_ndarray)
    heatmaps_avg = np.where(new_hp > decision_value)
...
def img_blur_2(img, heatmap, heatmaps_index, max_value):
    ...
    for index in range(len(heatmaps_index[0])):
        ...
        for i in range(c):
            img_np[y_st:y_end, x_st:x_end, i] = np.mean(
                img_npsample[y_min:y_max, x_min:x_max, i])
