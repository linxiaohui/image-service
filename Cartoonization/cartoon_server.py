...
class ImageCartoonizedHandler(tornado.web.RequestHandler, ABC):
    def get(self, gen_uuid):
        _conn = sqlite3.connect("image.db")
        _cursor = _conn.cursor()
        _cursor.execute("SELECT image_cartoonized FROM  upload_images WHERE uuid=?", (gen_uuid,))
        image = _cursor.fetchone()
        _conn.close()
        if image:
            content_type = imghdr.what(None, image[0])
...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        handlers = [
            ...
            (r"/image_cartoon/(.+)", ImageCartoonizedHandler),
...
if __name__ == "__main__":
    try:
        ...
        conn.execute("""CREATE TABLE upload_images(image_id INTEGER PRIMARY KEY AUTOINCREMENT, uuid TEXT, 
                                                   file_name TEXT, image_data BLOB, image_cartoonized BLOB,
